...
public abstract class Node extends GraphElement {
	...
	/**
	 * @return the point that represents the current position of this node; called
	...
	/**
	 * @return all edges incident on this node, even deleted ones; Caution: returns
	...
	/**
	 * The fixed versions, getFixedPosition() and setFixedPosition() are used within
	 * the Galant software to access/modify positions of nodes during parsing of
	 * GraphML input, in response to mouse dragging, or when the force directed
	 * heuristic is applied to reposition nodes. During execution of an algorithm, a
	 * position set by the algorithm takes precedence, i.e., a node moved by the
	 * algorithm can no longer be moved via dragging. However, any node that is
	 * *not* moved by the algorithm *can* still be moved by the user.
	 *
	 * The "right way to handle this is to have the algorithm declare whether or not
	 * it intends to move nodes -- the hook for that is already there -- and use
	 * this information to allow arbitrary position changes by the user during
