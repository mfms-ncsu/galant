...
\Code{removeUpdate} events for \Code{GEditorPanel}.
This works fine if the user is editing text directly or editing a graph in
the graph window and changes are pushed to the text panel.
However, insert updates also occur during GraphML parsing
when a graph is being read from a file or when an algorithm animation
modifies a graph in any way; the only modification that should trigger
\Code{setDirty} during animation is when a user moves a node.
...
% [Last modified: 2017 03 07 at 17:07:56 GMT]
