...
The first situation above is handled by a \Code{setEditMode(false)} in two
places: (i)~in the constructor for \Code{GGraphEditorPanel} (to forstall any
dirty status until the text panel is fully initialized); and (ii)~right
before a \Code{textPane.setText} when returning to edit mode from an
animation (to forstall dirty status until text panel is refreshed).
Edit mode is reset to true \emph{after} a property change in
\Code{GGraphEditorPanel} when it's not the one returning from an
animation.~\footnote{
  Apparently a return from animation or text panel initialization generates
  two property changes.
}

The second situation (change in directedness) is also handled in two places,
this time in \Code{GraphWindow}:
(i) a press of the directedness button does not generate a
\Code{pushToTextEditor} -- see \Code{actionPerformed} for the
\Code{Directedness} toggle button; and
(ii)~edit mode is disabled at the beginning of
\Code{changeDirectedness} -- this takes care of situations where change in
directedness is not the result of a button press.\footnote{
  An resulting bug/quirk is that, if an algorithm sets directedness, at least
  two node movements are required before the graph text becomes dirty.
}
...
% [Last modified: 2017 03 08 at 21:27:36 GMT]
