		...
		try {
	        ...
	        for(String graphMLKeyword : allGraphMLKeywords) {
	        	...
	        	while((index = content.indexOf(graphMLKeyword, index)) != -1) {
	        		...
	        		Character next = (index+graphMLKeyword.length() < content.length()-1) ? content.charAt(index+graphMLKeyword.length()) : ' ';
	        		if(!Character.isLetter(prev) && !Character.isLetter(next) && !prev.equals('.') && !next.equals('.'))
	        			doc.setCharacterAttributes(index, graphMLKeyword.length(), doc.getStyle(graphMLKeywordStyleName), true); 
