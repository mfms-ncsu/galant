...
public class AlgorithmExecutor {
    ...
    /**
     * Makes a note of the algorithm and its synchronizer and creates a thread
    ...
    /**
     * Needed for code that relies on knowing what the current display is
     * showing
     */
    ...
    public synchronized void incrementDisplayState () {
        ...
        if ( displayState == algorithmState && !synchronizer.algorithmFinished() && !synchronizer.stopped()
            ...
            do {
                try {
                    ...
                    if ( timeInBusyWait % PRINT_INTERVAL == 0 ) {
                        ...
                        /**
                         * @todo despite the synchronization, the status label
                         *       fails to get updated here
                         */
                        synchronized ( window ) {
            ...
            /**
             * there two types of exception thrown here: synchronizer when an
             * exception is displayed in a popup this whenever a GalantException
             * is thrown during execution not clear that both are needed, but
             * the synchronizer one is a backstop
             */
