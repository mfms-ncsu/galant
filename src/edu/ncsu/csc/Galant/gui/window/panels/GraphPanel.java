...
public class GraphPanel extends JPanel {
	...
	/**
	 * Maximum width of a node boundary or edge that can be specified by a spinner
	 * in the preferences panel
	 */
	public static final int MAXIMUM_LINE_WIDTH = 7;

	/**
	 * default width for a highlighted or colored edge or node boundary and for a
	 * selected edge during editing
	 */
	...
	/**
	 * Node weights and labels are to the right of their nodes. Weights appear above
	 * labels.
	 *
	 * Edge weights are to the left of labels. The (vertical) line separating
	 * weights from labels intersects the midpoint of the edge. Centers of labels
	 * and weights are aligned vertically with the (horizontal) line that intesects
	 * the midpoint of the edge. Edge weights and labels are inside bordered
	 * rectabgles.
	 *
	 * Node labels/weights are on top of a (white) filled rectangle with padding all
	 * around.
	 *
	 * In the relevant code, the position of each label/weight is recorded in the
	 * appropriate variable: it is the position of the top left corner of the label
	 * or weight without padding. Since strings are drawn with respect to the
	 * <emph>lower</emph> left corner, the font height must be added when the
	 * label/weight is drawn.
	 */
	...
	/**
	 * Holds the width to draw edges and node boundaries. Pulled on each repaint
	 * from the Galant Preferences
	 */
	private int defaultThickness = DEFAULT_WIDTH;

	/**
	 * Holds the width to draw edges and node boundaries when these are highlighted
	 * or colored
	 */
	private int highlightThickness = DEFAULT_HIGHLIGHT_WIDTH;

	/**
	 * Holds the radius for drawing nodes. Pulled on each repaint from the Galant
	 * Preferences
	 */
	...
	/**
	 * @return a "nice" string version of a double: no decimal point if it's an
	 *         integer, 'inf' if it's infinity, and only two decimal places
	 *         otherwise
	 */
	...
	public void drawGraph(Graph graph, Graphics2D g2d, int state) throws GalantException{
		...
		for (Edge e : edges){
			if ( e.inScope(state) && ! e.isHidden(state) && ! e.getSource().isHidden(state)
	...
	/**
	 * @todo This should not need to be a special case; a graph is always in some
	 *       state
	 */
	public void drawGraph(Graph graph, Graphics2D g2d) throws GalantException{
		Timer.drawingTime.start();
		List<Node> nodes = null;
		List<Edge> edges = null;
		...
		// Draw edges first to put them behind nodes
		for (Edge e : edges){
			if ( e.inScope(0) && ! e.isHidden() && ! e.getSource().isHidden() && ! e.getTarget().isHidden() ){
				drawEdge(graph, e, g2d);
			}
		}
	...
	/**
	 * @return true if the label of the node should be visible; the answer is
	 *         controlled by toggle switches when not in animation mode and by the
	 *         algorithm otherwise.
	 */
	private boolean labelVisible(Node node){
		...
		int state = dispatch.getDisplayState();
		boolean visible = node.hasLabel(state) && ! (node.getLabel(state).length() == 0) && ! node.labelIsHidden(state);
		return visible;
	}

	/**
	 * @return true if the weight of the node should be visible; the answer is
	 *         controlled by toggle switches when not in animation mode and by the
	 *         algorithm otherwise.
	 */
	private boolean weightVisible(Node node){
		...
		visible = visible && GraphDisplays.NODE_WEIGHTS.isShown() && ! node.weightIsHidden(state);
		return visible;
	}

	/**
	 * @return true if the label of the edge should be visible; the answer is
	 *         controlled by toggle switches when not in animation mode and by the
	 *         algorithm otherwise.
	 */
	private boolean labelVisible(Edge edge){
		int state = dispatch.getDisplayState();
		boolean visible = edge.hasLabel(state) && ! (edge.getLabel(state).length() == 0);
		visible = visible && GraphDisplays.EDGE_LABELS.isShown() && ! edge.labelIsHidden(state);
		return visible;
	}

	/**
	 * @return true if the weight of the edge should be visible; the answer is
	 *         controlled by toggle switches when not in animation mode and by the
	 *         algorithm otherwise.
	 */
	private boolean weightVisible(Edge edge){
		...
		visible = visible && GraphDisplays.EDGE_WEIGHTS.isShown() && ! edge.weightIsHidden(state);
		return visible;
	}

	/**
	 * @return the point at the center of node n, based on whether or not you're in
	 *         animation mode or whether the graph is layered. Edited by 2021 Galant
	 *         Team
	 * @todo !!! [Senior Design Team] !!! This is *the* place where the distinction
	 *       between logical and physical position needs to be handled
	 */
...
//				y = VERTICAL_PADDING + n.getLayer() * layerGap;
...
//		return nodeCenter;
	...
	/**
	 * Draws the specified node and its properties to the screen the positions of
	 * nodes to be drawn are determined by their state dependent position, which may
	 * not be the best solution since only one position that going through the
	 * algorithm is used if algorithm don't move nodes in the middle of execution,
	 ...
	 */
	private void drawNode(Node n, Graphics2D g2d) throws GalantException{
		...
		if ( labelVisible(n) ){
			...
			if ( ! label.trim().equals("") ){
				TextLayout layout = new TextLayout(label, NODE_LABEL_FONT, g2d.getFontRenderContext());
				...
				Point labelPosition = new Point(nodeCenter.x + nodeRadius + NODE_LABEL_DISTANCE + LABEL_PADDING,
						nodeCenter.y + LABEL_PADDING);
				g2d.setColor(Color.WHITE);
				g2d.fillRect(labelPosition.x - LABEL_PADDING, labelPosition.y - LABEL_PADDING,
						(int) (bounds.getWidth() + 2 * LABEL_PADDING), (int) (bounds.getHeight() + 2 * LABEL_PADDING));
				g2d.setColor(Color.BLACK);
				// text is anchored at *lower* left corner
				layout.draw(g2d, (float) labelPosition.getX(), (float) (labelPosition.getY() + bounds.getHeight()));
			}
		} // end, draw node label

		if ( weightVisible(n) ){
			String weight = doubleToString(n.getWeight(stateNumber));
			TextLayout layout = new TextLayout(weight, NODE_WEIGHT_FONT, g2d.getFontRenderContext());
			Rectangle2D bounds = layout.getBounds();
			// padding is 'shared' with node label
			Point weightPosition = new Point(nodeCenter.x + nodeRadius + NODE_LABEL_DISTANCE + LABEL_PADDING,
					nodeCenter.y - (int) (bounds.getHeight()));
			g2d.setColor(Color.WHITE);
			g2d.fillRect(weightPosition.x - LABEL_PADDING, weightPosition.y - LABEL_PADDING,
					(int) (bounds.getWidth()) + 2 * LABEL_PADDING, (int) (bounds.getHeight()) + 2 * LABEL_PADDING);
			g2d.setColor(Color.BLACK);
			// text is anchored at *lower* left corner
			layout.draw(g2d, (float) weightPosition.getX(), (float) (weightPosition.getY() + bounds.getHeight()));
		} // end, draw node weight

		/*
		 * Define node circle: used to create both outline and fill. Circle is filled
		 * first so that outline can be drawn on top of the filled circle
		 */
		Ellipse2D.Double nodeCircle = new Ellipse2D.Double(nodeCenter.x - nodeRadius, nodeCenter.y - nodeRadius,
				2 * nodeRadius, 2 * nodeRadius);

		/* draw node interior */
		if ( selectedNode != null && selectedNode.equals(n) && ! dispatch.isAnimationMode() ){
		...
		if ( displayIds ){
			...
			if ( idStr.length() > 1 ){
				g2d.drawChars(idStr.toCharArray(), 0, idStr.length(), nodeCenter.x - 8, nodeCenter.y + 4);
			} else{
				g2d.drawChars(idStr.toCharArray(), 0, idStr.length(), nodeCenter.x - 5, nodeCenter.y + 4);
			}
		}
	} // end, drawNode
	 ...
	 * @param g2d The graphics object used to draw the elements
	 */
	private void drawEdge(Graph g, Edge e, Graphics2D g2d) throws GalantException{
		int stateNumber = dispatch.getDisplayState();
		...
		if ( selectedEdge != null && selectedEdge.equals(e) && ! dispatch.isAnimationMode() ){
			Stroke selectedStroke = new BasicStroke(highlightThickness, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 0,
	...
	private void drawSelfLoopArrow(Point p1, Graphics2D g2d){
		...
		g2d.fillPolygon(new int[] { x, x - 6, x + 6, x }, new int[] { y, y + 6, y + 6, y }, 4);
	}
	 ...
	 * @param label  The label associated with the same edge. This is used to align
	 *               the weight with the label.
	 * @param source The source point of the relevant edge
	 * @param dest   The destination point of the relevant edge
	 * @param g2d    The graphics object used to draw the elements
	 */
	private void drawEdgeWeight(double weight, Point source, Point dest, Graphics2D g2d){
		/**
		 * @todo need special handling for self loops, i.e., put label and weight at the
		 *       bottom of loop
		 */

		String weightString = doubleToString(weight);

		Point2D edgeMiddle = new Point2D.Double(0.5 * (source.x + dest.x), 0.5 * (source.y + dest.y));

		TextLayout layout = new TextLayout(weightString, EDGE_WEIGHT_FONT, g2d.getFontRenderContext());
		Rectangle2D bounds = layout.getBounds();

		Point2D weightPosition = new Point2D.Double(edgeMiddle.getX() - LABEL_PADDING - bounds.getWidth(),
				edgeMiddle.getY() - 0.5 * bounds.getHeight());
		g2d.setColor(Color.WHITE);
		g2d.fillRect((int) (weightPosition.getX() - LABEL_PADDING), (int) (weightPosition.getY() - LABEL_PADDING),
				(int) (bounds.getWidth() + 2 * LABEL_PADDING), (int) (bounds.getHeight() + 2 * LABEL_PADDING));
		g2d.setColor(Color.BLACK);
		// border
		g2d.drawRect((int) (weightPosition.getX() - LABEL_PADDING), (int) (weightPosition.getY() - LABEL_PADDING),
				(int) (bounds.getWidth() + 2 * LABEL_PADDING), (int) (bounds.getHeight() + 2 * LABEL_PADDING));
		// text
		layout.draw(g2d, (float) weightPosition.getX(), (float) (weightPosition.getY() + bounds.getHeight()));
	}
	...
	private void drawEdgeLabel(String label, Point source, Point dest, Graphics2D g2d){
		...
		TextLayout layout = new TextLayout(label, EDGE_LABEL_FONT, g2d.getFontRenderContext());
		Rectangle2D bounds = layout.getBounds();

		Point2D labelPosition = new Point2D.Double(edgeMiddle.getX() + LABEL_PADDING,
				edgeMiddle.getY() - 0.5 * bounds.getHeight());
		g2d.setColor(Color.WHITE);
		g2d.fillRect((int) (labelPosition.getX() - LABEL_PADDING), (int) (labelPosition.getY() - LABEL_PADDING),
				(int) (bounds.getWidth() + 2 * LABEL_PADDING), (int) (bounds.getHeight() + 2 * LABEL_PADDING));
		g2d.setColor(Color.BLACK);
		// border
		g2d.drawRect((int) (labelPosition.getX() - LABEL_PADDING), (int) (labelPosition.getY() - LABEL_PADDING),
				(int) (bounds.getWidth() + 2 * LABEL_PADDING), (int) (bounds.getHeight() + 2 * LABEL_PADDING));
		// text
		layout.draw(g2d, (float) labelPosition.getX(), (float) (labelPosition.getY() + bounds.getHeight()));
	}
	 ...
	 * origin will line up with an edge between the specified points.
	 *
	...
	/**
	 * This method is used for user to select node, then change the property or
	 * position of selected node. Since this method only used in the situation that
	 * users are allowed to move nodes, which no change of positions caused by
	 * algorithm will happen, and there is no need for state dependent positions to
	 * be concerned, only the position associated with node itself need to be
	 * compared.
	 *
	...
	public Node selectTopClickedNode(Point p){
		...
		LogHelper.exitMethod(getClass(),
				"selectTopClickedNode, node = " + (selectedNode == null ? "null" : selectedNode.getId()));
		return top;
	}
	...
	public Node getSelectedNode(){
		...
		LogHelper.exitMethod(getClass(), "getSelectedNode, node = " + selectedNode);
