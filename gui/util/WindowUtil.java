...
public class WindowUtil
	{
			...
			public void actionPerformed(ActionEvent e)
				{
					GEditorFrame.getSingleton().dispatchEvent(
						new WindowEvent(GEditorFrame.getSingleton(), WindowEvent.WINDOW_CLOSING));
				}
		};
		...
		/**
		 * Keeps track of the given window's location and dimensions, so modifications made by the
		 * user are preserved between sessions. Note that the window's
		 * {@linkplain Window#setName(String) name} is used to identify it, so it is recommended to
			...
			final int defaultY, final int defaultWidth, final int defaultHeight)
			{
				window.addComponentListener(new ComponentAdapter(){
					...
					public void componentShown(ComponentEvent e)
						{
							Rectangle rect =
								new Rectangle(retrieveValue(window, X, defaultX), retrieveValue(
									window, Y, defaultY),
		...
		public static void linkWindows()
			{
				final JFrame graphFrame = GraphWindow.getGraphFrame(), editorFrame = GEditorFrame.getSingleton();
