 ...
 *
 */
public abstract class GEditorPanel extends JPanel implements DocumentListener,
      ...
      public String getToolTipText(MouseEvent me) {
        ...
        try {
          ...
          if ( as.containsAttribute(StyleConstants.Foreground,
            ...
            while ( loc >= 0 &&
                    Character.isJavaIdentifierPart( content.charAt(loc) ) ) loc--;
            loc++;
            int begin = loc;
            while ( loc < content.length() &&
                    Character.isJavaIdentifierPart( content.charAt(loc) ) ) loc++;
            int end = loc;
            return GAlgorithmSyntaxHighlighting.APIdictionary.get( content.substring(begin,
                                                                                     end) );
          }
        } finally { } return null;
      }
  ...
  public void insertUpdate(DocumentEvent arg0) {
    ...
    setDirty(true);
    if ( arg0.getChange( textPane.getStyledDocument().getDefaultRootElement() ) instanceof
         AbstractDocument.ElementEdit ) {
      AbstractDocument.ElementEdit ee =
        ( (AbstractDocument.ElementEdit)arg0.getChange( textPane.getStyledDocument().
      ...
      if ( ee.getChildrenAdded().length > 0 &&
        ...
        try { SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
              try { for ( int ii = 0; ii < fNumTabs; ii++ )
                      textPane.getStyledDocument().insertString(branchPoint,
                                                                "\t",
                                                                null);
  ...
  public void setFontSize(Integer size) {
    if ( size != null && size > 0 ) {
      ...
      if ( lineNumbers !=
           null ) lineNumbers.setFont( new Font(Font.DIALOG, Font.PLAIN, size) );
    }
  }
