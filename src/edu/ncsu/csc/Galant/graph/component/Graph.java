...
public class Graph {
     ...
     * edit mode or there is a redo operation, decremented when there is an undo
     ...
     *       handled; perhaps a graph can be another GraphElement; however,
    ...
    /**
     * @return true if user moved a node since last resetUserNodeMove(); called
    ...
    /**
     * @return a new state for this graph; the new state will be identical to
     ...
     * The cloned graph is modified by an animation so that the current edit
     * state remains unaffected. Nodes and edges of the current graph are reset
     * to their current edit state.
     *
     * @param currentGraph
     *            the graph to be accessed by the animation
     *
     * @todo !!! [Senior Design] !!! This crude mechanism of copying the current
     *       edit state of everything to start an algorithm is fraught with
     *       peril. As far as I can tell, the algorithm works on the deep copy
     *       of the graph, while the graph panel (user interface) knows only
     *       about the original edit graph. Since each node and edge is
     *       deep-copied, there is danger of an edit operation manipulating the
     *       wrong copy. This does not appear to be a problem when user moves a
    ...
    /**
     * Used to retrieve node positions from the graph copy created for algorithm
     ...
     * graph, where most recent is defined relative to a given time stamp, as
     * defined by forward and backward stepping through the animation.
     *
     * @see edu.ncsu.csc.Galant.algorithm.AlgorithmExecutor
     * @param stateNumber
     *            the numerical indicator (timestamp) of a state, usually the
     *            current display state
     * @return the latest instance of GraphState that was created before the
     ...
     * algorithm execution. Typically, an algorithm will declare its intent at
     * the beginning.
     */
     ...
     *         allocating an array of edges; unlike the case of nodes, it should
    ...
    /**
     * @return a list of all nodes in the graph, even ones that have been
    ...
    /**
     * @return a list of all edges in the graph, even ones that have been
     *         deleted used when drawing, so that we can see the effect of
     *         undoing deletions
     */
    ...
    /**
     * @return an edge with the given source and target if one exists; throws an
    ...
    /**
     * Deletes an edge with the given source and target if one exists; throws an
     * exception otherwise; if the graph is directed, source and target must
    ...
    /**
     * Deletes (but does not permanently remove) the specified <code>Node</code>
     ...
     *         execution; the assumption here is that the algorithm has to
     *         "know" the position of the node it is adding. The only difference
    ...
    /**
     * Adds a new edge to the graph with the specified source and target. Starts
     * an algorithm step if appropriate. Note: for undirected graphs, "source"
     * and "target" are meaningless.
     *
    ...
    /**
     * Adds a new edge to the graph with the specified source and target. Starts
     * an algorithm step if appropriate.
     *
     * @param sourceId
     *            the ID of the source <code>Node</code>
     * @param targetId
     *            the ID of the target <code>Node</code>
     *
     *            Note: for undirected graphs, "source" and "target" are
     *            meaningless.
     * @return the added edge
     *
    ...
    /**
     * Adds a new edge to the graph with the specified source and target. Note:
     * for undirected graphs, "source" and "target" are meaningless.
     *
     * @return the added edge
     *
     ...
     *
     * @param n
     *            the <code>Node</code> to remove; this one actually removes the
    ...
    /**
     * Call the repositioning algorithm Currently the only supported algorithm
     * is Force directed repositioning
     *
     * @param boost
     *            is the extent to which degree of a node will cause it to repel
     *            other nodes - repulsive force is multiplied by degree raised
     *            to the boost power.
     *
