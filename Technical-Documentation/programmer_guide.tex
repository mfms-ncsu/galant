...
The \Code{display} and \Code{print} functions also take arbitrary objects as
...
Nodes are more complicated: \Code{hide($v$)} effectively hides the incident
edges of $v$,
but \Code{show($v$)} restores only the visibility of those edges
...
element versions. The idea behind \Code{showEdges} and \Code{showNodes} is to
undo hiding of all relevant graph elements.  The function \Code{showEdges}
...
\Code{showNodes} will not necessarily undo the effects of hiding each node.
Recall that when you do \Code{hide($v$)}, where $v$ is a node, all of $v$'s
incident edges are hidden as well.  The singular \Code{show($v$)}, however,
does not undo the hiding all of $v$'s incident edges, only of those
\emph{whose visibility was restored in the interim}.
Therefore \Code{showNodes} makes visible only those edges that were made
...
% [Last modified: 2017 01 22 at 00:45:07 GMT]
